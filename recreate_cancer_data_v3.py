...
def recreate_cancer_data_v3():
    ...
    for _, row in df_clean.iterrows():
        ...
        if gender_kr in gender_mapping and age_kr in age_mapping:
            ...
            # 5개년 평균 발생률 (숫자로 변환)
            try:
                avg_5year = float(row['5개년 평균']) if pd.notna(row['5개년 평균']) else 0.0
                rate_2022 = float(row['2022년']) if pd.notna(row['2022년']) else avg_5year
    ...
    for gender in cancer_data:
        for age_group in cancer_data[gender]:
            ...
            # 2022년 발생률 기준으로 정렬 (숫자만 비교)
            cancer_data[gender][age_group].sort(key=lambda x: float(x["rate"]) if isinstance(x["rate"], (int, float)) else 0, reverse=True)
