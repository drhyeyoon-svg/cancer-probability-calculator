...
# -*- coding: utf-8 -*-
"""
엑셀 파일을 파싱하여 JavaScript 데이터 파일로 변환하는 스크립트 (수정판)
실제 엑셀 파일 구조에 맞춰 파싱 로직 개선
"""
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if pd.notna(cancer_name) and cancer_name != '24개 암종별':
                    current_cancer = cancer_name
                    continue
                
                # 암종명이 없으면 현재 암종 사용
                if pd.isna(current_cancer) or current_cancer == '모든 암(C00-C96)':
                    continue
                
                # 성별이 NaN이거나 '계'인 경우 건너뛰기
                if pd.isna(gender) or gender == '계':
                    continue
                ...
                # 성별 매핑
                gender_key = gender_mapping.get(gender)
                if not gender_key:
                    continue
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                age = row['연령별']
                male_rate = row['2023.1']  # 사망확률(남자) (%)
                female_rate = row['2023.2']  # 사망확률(여자) (%)
